...
ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC = "openrouter_exec"
ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC_FAILED = "openrouter_exec_failed"
ROUTER_COMPLETION_SOURCE_ROUTER_ALL_FAILED = "router_all_failed"

# Backward-compat aliases for historical rows.
ROUTER_COMPLETION_SOURCE_CODEX_EXEC = ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC
ROUTER_COMPLETION_SOURCE_CODEX_EXEC_FAILED = ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC_FAILED
ROUTER_COMPLETION_SOURCE_LAUNCHED_SUPERVISED = ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC_FAILED
ROUTER_COMPLETION_SOURCE_AG_LAUNCH_FAILED = ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC_FAILED
ROUTER_COMPLETION_SOURCE_OLLAMA_EXEC = ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC
ROUTER_COMPLETION_SOURCE_OLLAMA_EXEC_FAILED = ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC_FAILED
...
ROUTER_COMPLETION_SOURCE_CONTRACT = frozenset(
    {
        ...
        ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC,
        ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC_FAILED,
...
def _update_execution_idempotent(
    ...
    *,
    ...
    def _apply() -> dict[str, Any]:
        return update_request_execution(projects_root, project_id, request_id, **kwargs)
...
def _cost_rates(settings: dict[str, Any] | None) -> dict[str, tuple[float, float]]:
    default_rates = {"openrouter": (0.010, 0.030)}
    if not isinstance(settings, dict):
    ...
    cfg = provider_rates.get("openrouter")
    if isinstance(cfg, dict):
        try:
            in_rate = float(cfg.get("input", out["openrouter"][0]))
        except (TypeError, ValueError):
            in_rate = out["openrouter"][0]
        try:
            out_rate = float(cfg.get("output", out["openrouter"][1]))
        except (TypeError, ValueError):
            out_rate = out["openrouter"][1]
        out["openrouter"] = (max(0.0, in_rate), max(0.0, out_rate))
    return out
...
def _emit_cost_event(
    ...
    *,
    ...
    cost_cad_estimate = ((input_tokens * in_rate) + (output_tokens * out_rate)) / 1000.0
...
def route_action(
    ...
    *,
    ...
    execution_mode = str(automation.get("execution_mode") or action.execution_mode or "openrouter_headless")
    openrouter_cfg = automation.get("openrouter") if isinstance(automation.get("openrouter"), dict) else {}
    timeout_cfg = automation.get("timeouts") if isinstance(automation.get("timeouts"), dict) else {}
    openrouter_timeout = int(timeout_cfg.get("openrouter_seconds", timeout_cfg.get("codex_seconds", 900)))
    ...
    if not policy_decision.allowed:
        ...
        _record_execution_event(
            ...
            action.request_id,
            ROUTER_COMPLETION_SOURCE_POLICY_DENIED,
            {"agent_id": action.agent_id, "reason_code": policy_decision.reason_code},
        )
        return ExecutionResult(
            request_id=action.request_id,
            agent_id=action.agent_id,
    ...
    if not bool(openrouter_cfg.get("enabled", True)):
        return ExecutionResult(
            request_id=action.request_id,
            agent_id=action.agent_id,
            project_id=project_id,
            ...
            error="openrouter automation disabled",
        )

    retry_cfg = openrouter_cfg.get("retry") if isinstance(openrouter_cfg.get("retry"), dict) else {}
    ...
    result = run_openrouter(action.prompt_text, cwd=projects_root / project_id, timeout_s=openrouter_timeout)
    attempt = 1
    _record_execution_event(
        projects_root,
        project_id,
        action.request_id,
        "openrouter_attempt",
        {"attempt": attempt, "status": result.status, "success": result.success, "error": result.error},
    )
    ...
    if cost_emit_error:
        _record_execution_event(
            ...
            action.request_id,
            "cost_event_dropped",
            {"provider": "openrouter", "reason": cost_emit_error},
        )

    completion_source = (
        ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC if close_request else ROUTER_COMPLETION_SOURCE_OPENROUTER_EXEC_FAILED
    )
    _update_execution_idempotent(
        projects_root,
        project_id,
    ...
    _record_execution_event(
        ...
        project_id,
        action.request_id,
        "openrouter_execution_result",
        {"attempts": attempt, "status": result.status, "close_request": close_request, "error": result.error},
    )
    ...
    if close_request:
        finalize_run_bundle(
            ...
            tool_calls=[{"runner": "openrouter", "attempts": attempt, "timeout_s": openrouter_timeout}],
            outputs_payload={
            ...
            verdict="success",
            tx_id=f"{action.request_id}:bundle:openrouter_final",
        )
        return ExecutionResult(
            request_id=action.request_id,
            agent_id=action.agent_id,
    ...
    _update_execution_idempotent(
        ...
        kwargs={
            ...
            "close_request": False,
            "error": result.error or "openrouter execution failed",
        },
    )
